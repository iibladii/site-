...
public interface TelephoneRepository extends CrudRepository<Telephone, Long> {
	...
	/**
	 * Выборка на основе заданных параметров (Предполагая что subdivision == null)
	 * @param number
	 * @param att1
	 * @param att2
	 * @param room
	 ...
	 * @return результаты выборки
	 */
	@Query("SELECT distinct p FROM Telephone p, in(p.department) t, in(p.adsl) a WHERE p.att1 like :att1 and p.att2 like :att2 and p.number like :number and p.room like :room and t.name like :department and a.name like :adsl and p.isDel != true")
    public List<Telephone> find_NoSubdivision(@Param("number") String number,@Param("att1") String att1, @Param("att2") String att2, @Param("room") String room, @Param("department") String department, @Param("adsl") String adsl);
	 ...
	 * @return результаты выборки
	 */
	@Query("SELECT distinct p FROM Telephone p, in(p.department) t, in(p.adsl) a WHERE p.att1 like :att1 and p.att2 like :att2 and p.number like :number and p.room like :room and t.name like :department and a.name like :adsl and p.isDel != false")
    public List<Telephone> findDel_NoSubdivision(@Param("number") String number,@Param("att1") String att1,@Param("att2") String att2,@Param("room") String room,@Param("department") String department,@Param("adsl") String adsl);
