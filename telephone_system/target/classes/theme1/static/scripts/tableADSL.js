...
function loadADSLTable(elem){
	$.get("/ajax/adsl_info?name="+encodeURIComponent(document.getElementById("ads_name_").value)+'&elem='+encodeURIComponent(elem),function(data,status){
...
function loadADSLTableDel(elem){
	$.get("/ajax/adsl_info?name="+encodeURIComponent(document.getElementById("ads_name_").value)+'&elem='+encodeURIComponent(elem),function(data,status){
		...
		var adslList='<table border="1" id="usersTable">'+
			'<thead>'+
				'<tr>'+
					'<th>ADSL&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp</th><th>&nbsp&nbsp</th>'+
				'</tr>'+
			'</thead>';
			for(var i=0;i < parseInt(data.roleList.length); i++){
...
function loadInfo(str){
	...
	'<div>'+
		'&nbsp;'+
		'<div>'+
			'<div class="informationL">ADSL:</div>'+
...
$(document).on('click','#btn',function(){
	...
	if(flag==0){
		...
		flag=0;
			...
			$.get("/adsl/adsl_del?name=" + encodeURIComponent(param) ,function(data,status){
				if (!(status == "success")) {
		    		  // обработать ошибку
		    		  ...
		    		  // вывести результат
		    		  var rsp = data;
		    		  //alert( rsp ); // responseText -- текст ответа.
...
$(document).on("click", ".page-с", function (){
  ...
  //Обработка нажатия кнопки сохранить
    $(document).on("click", "#save", function() {
    	...
    	xhr.open("GET", '/adsl/adsl_update?' + encodeURIComponent(params), false);
    	...
    	/////////////////////////////////////////////////////
    	//$.get("/ajax/adsl_info?name="+document.getElementById("ads_name_").value+"&oldName=",function(data,status){
    	//	});
    	//document.getElementById("ads_name_").value = 'fgdgdfg';
  ...
  //Обработка нажатия кнопки создать
    ...
    $(document).on("click", "#vvod", function() {
    	if(document.getElementById("adsl_").value!=""){
    		$.get("/ajax/adsl_create?adsl=" + encodeURIComponent(document.getElementById("adsl_").value),function(data,status){
    			var resp_vvod = document.getElementById("response_vvod");
    			if(data == "success")
    				resp_vvod.innerHTML='<p style="color:#005500">Вставка нового ADSL успешно завершена</p>';
    			else
    				if(data == "entry more then zero")
    					resp_vvod.innerHTML='<p style="color:#550000">Такой ADSL уже есть в базе</p>';
    				else
